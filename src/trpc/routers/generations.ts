  ...
  create: orgProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!voice.r2ObjectKey) {
        throw new TRPCError({ code: 'PRECONDITION_FAILED', message: 'Voice audio not available' })
      }
      ...
      if (!(data instanceof ArrayBuffer)) {
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Invalid audio response' })
      }
